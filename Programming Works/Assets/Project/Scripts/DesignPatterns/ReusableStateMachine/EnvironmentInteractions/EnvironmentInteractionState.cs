...
{
	// when each concrete state instantiated, it will require the context to be passed as an argument
	protected EnvironmentInteractionContext Context;

	protected EnvironmentInteractionState(EnvironmentInteractionContext context,
	...
	protected void StartIkTargetPositionTracking(Collider intersectingCollider)
	{
		...
		Context.CurrentIntersectingCollider == null)
		{
			...
			Vector3 closesPointFromRoot = GetClosestPointOnCollider(intersectingCollider, Context.RootTransform.position);
			Context.SetCurrentSide(closesPointFromRoot);
